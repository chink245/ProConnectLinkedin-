...
import { getAllPosts , createPost, deletePost, getAllComments, postComment } from '@/config/redux/action/postAction';
...
{postContent.length > 0 && 
  <div onClick={handleUpload} className={styles.uploadButton}>Post</div>
    ...
    <div className={styles.postsContainer}>
      {postState.posts.map((posts) => {
  return (
    <div key={posts._id} className={styles.singleCard}>
...
<div onClick={async ()=>{
  await dispatch(deletePost({post_id: posts._id}))
  await dispatch(getAllPosts())
}}style={{cursor:"pointer"}}>
<svg style={{height:"1.4rem",color:"red",justifyContent:"flex-end"}} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-6">
...
<p style={{color:"grey"}}>@{posts.userId.username}</p>
<p style={{paddingTop:"1.3rem"}}>{posts.body}</p>
<div className={styles.singleCard_image}>
  {posts.media !== "" ? <img src={`${BASE_URL}/${posts.media}`}/> : <></>}

</div>

<div className={styles.optionsContainer}>
  <div onClick={async ()=>{
    await dispatch(incrementPostLike({post_id:posts._id}))
    dispatch(getAllPosts())
  ...
  <div onClick={()=>{
    dispatch(getAllComments({post_id:posts._id}))
  }}className={styles.singleOption_optionsContainer}>
    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-6">
  ...
  <div onClick={()=>{
    const text = encodeURIComponent(posts.body)
     ...
     </div>
     {
      ...
      className={styles.commentsContainer}>
        ...
        <div className={styles.postCommentContainer}>
          <input type='' value={commentText} onChange={(e)=>setCommentText(e.target.value)} placeholder='Comments'/>
          <div onClick={async()=>{
